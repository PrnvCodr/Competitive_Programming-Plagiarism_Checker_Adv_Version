        ...
        for uploaded_file in files_to_process:
            try:
                ...
                if file_size < min_file_size:
                    st.sidebar.warning(f"⚠️ {uploaded_file.name} too small ({file_size} bytes)")
                    skipped_count += 1
                    continue
                elif file_size > max_file_size * 1024:
                    st.sidebar.warning(f"⚠️ {uploaded_file.name} too large ({file_size/1024:.1f} KB)")
                    skipped_count += 1
                    continue
                elif len(content.strip()) == 0:
                    st.sidebar.warning(f"⚠️ {uploaded_file.name} is empty")
                    skipped_count += 1
                    continue
        ...
        if file_count >= 2:
            ...
            if file1 != file2:
                ...
                for i, (alg, sim) in enumerate(similarities.items()):
                    with cols[i % 3]:
                        color = "🔴" if sim >= 0.8 else "🟡" if sim >= 0.6 else "🟢"
